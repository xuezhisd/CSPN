...
class Affinity_Propagate(nn.Module):
    ...
    def forward(self, guidance, blur_depth, sparse_depth):
        ...
        # sparse point depth & predict depth
        ...
        for i in range(16):
            ...
            result_depth = self.max_of_8_tensor(elewise_max_gate1, elewise_max_gate2, elewise_max_gate3,
                                                elewise_max_gate4, elewise_max_gate5, elewise_max_gate6,
    ...
    def eight_way_propagation(self, weight_matrix, blur_matrix, kernel):
        ...
        # mean conv & init weight & no grad
        self.avg_conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=kernel, stride=1, padding=(kernel - 1) / 2,
                                  bias=False)
        ...
        # sum conv * init weight & no grad
        self.sum_conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=kernel, stride=1, padding=(kernel - 1) / 2,
                                  bias=False)
